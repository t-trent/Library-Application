...
def pay_fine():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if fine:
                ...
                """, (fine_id,))
                conn.commit()
                success_message = f"Fine [ID: {fine_id}] of ${fine['amount']:.2f} has been successfully paid!"
            else:
    ...
    fines = conn.execute("""
        SELECT F.fine_id, I.title, I.item_type, P.name AS borrower_name, P.person_id AS borrower_id, F.amount AS fine_amount
        FROM Fines F
